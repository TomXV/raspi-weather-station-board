 ...
 */
function tempDeltaFrom30mAgo(data: any): number {
...
class I18n {
  ...
  /** 現在言語の翻訳テキスト一式 */
  get t(): LangText { return this.ui[this._lang]; }
  ...
  display(loc: Loc): { name: string; region: string } {
    const l = this._lang;
    const name =
      l === 'en' ? (loc.name_en || loc.name_ja) :
      l === 'zh' ? (loc.name_zh || loc.name_en || loc.name_ja) :
      l === 'ko' ? (loc.name_ko || loc.name_en || loc.name_ja) :
      loc.name_ja;
    const region =
      l === 'en' ? (loc.region_en || loc.region_ja) :
      l === 'zh' ? (loc.region_zh || loc.region_en || loc.region_ja) :
      l === 'ko' ? (loc.region_ko || loc.region_en || loc.region_ja) :
      loc.region_ja;
    return { name, region };
  }
  ...
  decodeWMO(code: number): WMOEntry {
    return this.wmoMap.get(code) ?? { code, icon: '❓', ja: `(${code})`, en: `(${code})`, ko: `(${code})` };
  }
...
class ClockPanel {
  ...
  tick(): void {
    ...
    if (this.elClock12) {
      this.elClock12.textContent = this.i18n.lang === 'en'
        ? `${ampm} ${h12}:00`
        : `${ampm}${h12}${t.hourUnit}`;
    }
  ...
  updateNextUpdate(now = new Date()): void {
    ...
    if (this.elNextUpd) this.elNextUpd.textContent = `${this.i18n.t.nextUpdate} ${mm}:${ss}`;
  }
  ...
  private formatDate(now: Date, cal: CalMode): string {
    ...
    try {
      if (cal === 'japanese') {
        return new Intl.DateTimeFormat('ja-JP-u-ca-japanese', { year: 'numeric', month: '2-digit', day: '2-digit' }).format(now);
      }
      if (cal === 'chinese') {
        return new Intl.DateTimeFormat('zh-CN-u-ca-chinese', { year: 'numeric', month: 'long', day: 'numeric' }).format(now);
      }
    } catch { /* Intl が未対応の場合は西暦を使用 */ }
    return greg;
  }
  ...
  private calendarMode(now: Date): CalMode {
    ...
    if (this.i18n.lang === 'zh') return (['gregory', 'japanese', 'chinese'] as const)[phase % 3];
...
class WeatherGrid {
  ...
  get nextRefreshAt(): number { return this._nextRefreshAt; }
  ...
  async refresh(): Promise<void> {
    ...
    await Promise.allSettled([...this.cities, ...this.nagano].map(async loc => {
      try {
    ...
    if (lastUpdEl) {
      lastUpdEl.textContent =
        `${this.i18n.t.updated}: ${String(now.getHours()).padStart(2,'0')}:${String(now.getMinutes()).padStart(2,'0')}`;
    }
  }
  ...
  renderNaganoPage(index: number): void {
    ...
    const rows = this.nagano.slice(this.naganoPage * NAGANO_PAGE_SIZE, (this.naganoPage + 1) * NAGANO_PAGE_SIZE);
  ...
  nextNaganoPage(): void { this.renderNaganoPage(this.naganoPage + 1); }
   ...
   */
  redrawAll(): void {
    this.cities.forEach(c => { if (this.cache[c.id]) this.updateCard(c, this.cache[c.id]); });
    this.renderNaganoPage(this.naganoPage);
  }

  /** Open-Meteo から 1 地点分の現況・日次・時間別データを取得する */
  private async fetchWeather(loc: Loc): Promise<any> {
  ...
  private updateCard(loc: Loc, data: any): void {
...
class WarningPanel {
  ...
  async fetch(): Promise<void> {
    try {
      const results = await Promise.allSettled(this.areas.map(async area => {
        ...
        if (!headline) return `${area.label_ja}: ${this.i18n.ui.ja.warningNoHeadline}`;
        return `${area.label_ja} [${reportTime}] ${headline}`;
      }));

      const lines = results.map((r, i) =>
        r.status === 'fulfilled' ? r.value : `${this.areas[i].label_ja}: ${this.i18n.ui.ja.warningFetchFailed}`
      );
      setTickerText('alert-track', lines.join(' ｜ '), this.i18n.t.noInfo);
    } catch {
      setTickerText('alert-track', this.i18n.ui.ja.warningAllFailed, this.i18n.t.noInfo);
    }
  }
}
...
class WeatherBoard {
  ...
  private applyLang(): void {
    ...
    ] as [string, string][]).forEach(([id, text]) => {
