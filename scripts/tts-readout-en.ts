...
async function buildTtsText(warningsOnly = false): Promise<string> {
  ...
  if (warningsOnly) {
    parts.push(
      `This is the weather warning report for ${month} ${day}, ${year}, around ${hour}:${minute}.`,
    );
  } else {
    parts.push(
  ...
  // 4. Warnings
  if (!warningsOnly) parts.push("Now, weather warnings and advisories.");
...
async function synthesizeChunk(text: string, outFile: string): Promise<void> {
  const proc = Bun.spawn(
    ["node", EDGE_TTS.binPath, "-v", "en-US-JennyNeural", "-l", "en-US", "--timeout", "30000", "-t", text, "-f", outFile],
